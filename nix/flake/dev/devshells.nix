    ...
    }:
    {
      devShells.default = config.legacyPackages.haskellPkgs.shellFor {
        ...
        shellHook = ''
          ...
          # Nix can't be recognized by cabal-install, while in Nix's own build process, the
          # build tools are invoked directly with `Setup.hs`, and in non-Nix development
          # environments, they are installed by cabal-install. Based on the workaround provided by
          ...
          # Generate cabal.project.freeze
          cabal v2-freeze --project-dir=$project_root
        '';
      };
    };
}
